...
namespace DigitalRuby.PyroParticles
{
    ...
    public class FireBaseScript : MonoBehaviour
    {
        ...
        public static void CreateExplosion(Vector3 pos, float radius, float force)
        {
            ...
            Collider[] objects = UnityEngine.Physics.OverlapSphere(pos, radius, mask);
            foreach (Collider h in objects)
            {
                if (h.transform.name == "Monster")
                {
                    h.GetComponent<MonsterController>().DamageTaken(PlayerController.fireDamage);
                }
                if (h.transform.name == "Boss")
                {
                    GameController.gameController.BossHP.value -= PlayerController.fireDamage;
                }
            }
        }
